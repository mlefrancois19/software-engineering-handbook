# 13 — Infrastructure (Cloud & On-Prem)

Infrastructure as code means there is no "click-ops by whoever is awake": every change goes through review and CI like application code, so the person who runs it never has to have been the person who built it (chapters 01, 09, 11). Game days run across time zones precisely because the runbooks are written down and the fixes are rebuilt, not improvised.
...
- Remote state with locking; plan output is reviewed in the PR; apply runs through CI. Drift between environments is a bug (chapter 11).
- Secrets never live in IaC code or state — injected from a secret manager at runtime (chapter 09).

## Immutable & disposable

- Everything is built once and replaced, never mutated in place: images are built in CI (chapter 11) and patched by rebuilding and redeploying.
...
- Degrade gracefully: circuit breakers and bulkheads keep one failing dependency from taking down the service (chapter 07).
...
- Automated image/rebuild cadence; critical CVEs get a fix SLA (chapter 09).
